...
class LCS_dynamic:
    ...
    def lcs_dynamic(self, m, n):
        ...
        recursive calls by visiting the array (table) systematically.       
        """
        
        for i in range(m+1):
            for j in range(n+1):
                if i==0 or j==0: #The first row and first column of the array should have "0s" so that we can start 
                                #the calculation of the lens
                    self.table[i][j] = 0
                elif self.seqA[i-1] == self.seqB[j-1]: #If the symbol of each sequence match
                    self.sum_count += 1
                    self.table[i][j] = self.table[i-1][j-1] + 1 #We add 1 to the len found in the previous cell of the array
                                                                #(last LCS len found), once we found a match
                else:
                    self.max_calc += 1
                    self.table[i][j] = max(self.table[i-1][j],self.table[i][j-1]) #We found if the max len is the left cell
    ...
    def traceback_table(self,m,n):
        ...
        if (self.seqA[m-1] == self.seqB[n-1]): #If the last char of each subsequence is equal, this char
    ...
    def get_lcs_longest_subsquence(self):
        ...
        for subsq in self.traceback_table(len(self.seqA), len(self.seqB)):
            print (subsq[::-1])
        end_time = time.time() - start_time
        print("\nLongest subsquence(s) found in %s seconds" % (round(end_time,7)))        
